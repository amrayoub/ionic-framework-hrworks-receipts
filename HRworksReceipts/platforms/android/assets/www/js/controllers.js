...
.controller('receiptCtrl', function ($scope, $localstorage, $location, $ionicViewService, $filter, $ionicActionSheet, $state, $ionicPopup, $ionicModal, $timeout, $stateParams) {
	...
	if ($stateParams.guid != "new") {
		...
		$scope.receiptTitle = "{{ 'EDIT_RECEIPT' | translate }}";
	} else {
		$scope.receiptTitle = "{{ 'NEWRECEIPT' | translate }}";
	}
	...
	$scope.showActionsheet = function () {
		$ionicActionSheet.show({
			titleText : "{{ 'OPTIONS' | translate }}",
			buttons : [{
					text : "<i class='icon ion-ios7-copy-outline'></i> {{ 'COPY' | translate }}"
				},
			],
			destructiveText : "{{ 'DELETE' | translate }}",
			...
			buttonClicked : function (index) {
				if (index == 0) {
					if (!$scope.form.text || !$scope.form.amount || !$scope.form.date
						...
						$ionicPopup.alert({
							title : "<b>{{ 'ERROR' | translate }}:</b>",
							content : "{{ 'COPYRECEIPT_ERROR' | translate }}"
						});
						return true;
					} else {
						if($localstorage.getObjects('hideAlert').hideAlert == true) {
							...
							var confirmPopup = $ionicPopup.show({
								title : "<b>{{ 'COPYRECEIPT' | translate }}</b>",
								template : "{{ 'COPYRECEIPT_INFO' | translate }}",
								scope : $scope,
								buttons : [{
